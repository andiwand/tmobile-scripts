...
def message(resource, warning, critical):
	...
	msg += "%s of \"%s\" already used. (%d / %d)" % (percent, resource[0], resource[1], resource[2])
	return code, msg

parser = argparse.ArgumentParser(description="Check T-Mobile single quota.")
...
try:
	resources = single.fetch(args.username, args.password)
except:
